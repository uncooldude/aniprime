    ...
    <div className="max-w-[1260px] mx-auto px-[15px] flex flex-col mt-[64px] max-md:mt-[50px]">
      <ul className="flex gap-x-2 mt-[50px] items-center w-fit max-[1200px]:hidden">
        <li className="flex gap-x-3 items-center">
          <Link to="/" className="text-white hover:text-[#3324f2] text-[17px]">
            Home
          </Link>
      ...
      <div className="flex flex-col gap-y-5 mt-6">
        <h1 className="font-bold text-2xl text-[#3324f2] max-[478px]:text-[18px]">
          Sort By Letters
        </h1>
          ...
          {[
            ...
            return (
              <Link
                to={`/az-list/${linkPath}`}
                key={index}
                className={`text-md bg-[#373646] py-1 px-4 rounded-md font-bold hover:text-black hover:bg-[#3324f2] hover:cursor-pointer transition-all ease-out ${
                  isActive ? "text-black bg-[#3324f2]" : ""
                }`}
              >
                {item}
              </Link>
